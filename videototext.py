...
async def process_video(file: UploadFile = File(...)):
    ...
    # Run the processing synchronously so we can return results
    result = process_video_task(file_bytes, file.filename)

    return JSONResponse(content=result)


def process_video_task(file_bytes: bytes, filename: str):
    """Runs video processing synchronously so results can be returned."""
    try:
